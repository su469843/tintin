//#region node_modules/edge-tts-universal/dist/browser.js
...
var _BrowserDRM = class _BrowserDRM {
	...
	static handleClientResponseError(response) {
		if (!response.headers) throw new SkewAdjustmentError("No headers in response.");
		const serverDate = response.headers["date"] || response.headers["Date"];
		if (!serverDate) throw new SkewAdjustmentError("No server date in headers.");
		const serverDateParsed = _BrowserDRM.parseRfc2616Date(serverDate);
		if (serverDateParsed === null) throw new SkewAdjustmentError(`Failed to parse server date: ${serverDate}`);
...
var EdgeTTSBrowser = class {
			...
			if (this.ws) {
				this.ws.onmessage = (event) => {
					if (typeof event.data === "string") {
						...
						if (headers.Path === "audio.metadata") try {
					...
					} else if (event.data instanceof Blob) event.data.arrayBuffer().then((arrayBuffer) => {
						const headerLength = new DataView(arrayBuffer).getUint16(0);
	...
	*/
	async connect() {
		...
		const url = `${WSS_URL}&Sec-MS-GEC=${await BrowserDRM.generateSecMsGec()}&Sec-MS-GEC-Version=${SEC_MS_GEC_VERSION}&ConnectionId=${connectionId}`;
		this.ws = new WebSocket(url);
		return new Promise((resolve, reject) => {
			if (!this.ws) return reject(/* @__PURE__ */ new Error("WebSocket not initialized"));
			this.ws.onopen = () => {
				resolve();
			};
			this.ws.onerror = (error) => {
...
function browserConnectId() {
	...
	return `${hex.slice(0, 8)}-${hex.slice(8, 12)}-${hex.slice(12, 16)}-${hex.slice(16, 20)}-${hex.slice(20, 32)}`.replace(/-/g, "");
}
...
var TTSConfig = class _TTSConfig {
	...
	*/
	...
	static validateStringParam(paramName, paramValue, pattern) {
		if (typeof paramValue !== "string") throw new TypeError(`${paramName} must be a string`);
		if (!pattern.test(paramValue)) throw new ValueError(`Invalid ${paramName} '${paramValue}'.`);
	}
};
...
function browserGetHeadersAndDataFromText(message) {
	...
	const headers = {};
	if (headerEndIndex !== -1) {
		const headerLines = messageString.substring(0, headerEndIndex).split("\r\n");
...
function browserGetHeadersAndDataFromBinary(message) {
	if (message.length < 2) throw new Error("Message too short to contain header length");
	...
	if (headerLength > 0 && headerLength + 2 <= message.length) {
		...
		const headerLines = new TextDecoder().decode(headerBytes).split("\r\n");
		for (const line of headerLines) {
			const [key, value] = line.split(":", 2);
			if (key && value) headers[key] = value.trim();
		}
...
function browserSplitTextByByteLength(text, byteLength) {
	return (function* () {
		...
		while (buffer.length > byteLength) {
			...
			if (lastNewline > 0) splitAt = new TextEncoder().encode(sliceText.substring(0, lastNewline)).length;
			else if (lastSpace > 0) splitAt = new TextEncoder().encode(sliceText.substring(0, lastSpace)).length;
			const chunk = buffer.slice(0, splitAt);
			const chunkText = new TextDecoder().decode(chunk).trim();
			if (chunkText) yield new TextEncoder().encode(chunkText);
			buffer = buffer.slice(splitAt);
		}
		const remainingText = new TextDecoder().decode(buffer).trim();
		if (remainingText) yield new TextEncoder().encode(remainingText);
	})();
}
var BrowserCommunicate = class {
	...
	*/
	constructor(text, options = {}) {
		...
		this.texts = browserSplitTextByByteLength(browserEscape(browserRemoveIncompatibleCharacters(text)), 4096);
	...
	parseMetadata(data) {
		...
		for (const metaObj of metadata["Metadata"]) {
			...
			if (metaType === "WordBoundary" || metaType === "SentenceBoundary") return {
				type: metaType,
				offset: metaObj["Data"]["Offset"] + this.state.offsetCompensation,
	...
	async *_stream() {
		...
		let timeoutId;
		if (this.connectionTimeout) timeoutId = window.setTimeout(() => {
			websocket.close();
		...
		websocket.onmessage = (event) => {
			...
			if (typeof data === "string") {
				...
				if (path === "audio.metadata") try {
					...
					websocket.close();
				} else if (path !== "response" && path !== "turn.start") messageQueue.push(new UnknownResponse(`Unknown path received: ${path}`));
			} else if (data instanceof ArrayBuffer) {
				const bufferData = BrowserBuffer.from(data);
				if (bufferData.length < 2) messageQueue.push(new UnexpectedResponse("We received a binary message, but it is missing the header length."));
				else {
					const [headers, audioData] = browserGetHeadersAndDataFromBinary(bufferData);
					if (headers["Path"] !== "audio") messageQueue.push(new UnexpectedResponse("Received binary message, but the path is not audio."));
					else if (headers["Content-Type"] !== "audio/mpeg") {
						if (audioData.length > 0) messageQueue.push(new UnexpectedResponse("Received binary message, but with an unexpected Content-Type."));
					} else if (audioData.length === 0) messageQueue.push(new UnexpectedResponse("Received binary message, but it is missing the audio data."));
					else messageQueue.push({
						type: "audio",
						data: audioData
					});
				}
			} else if (data instanceof Blob) data.arrayBuffer().then((arrayBuffer) => {
		...
		await new Promise((resolve, reject) => {
			...
			if (this.connectionTimeout) setTimeout(() => {
		...
		websocket.send(`X-Timestamp:${browserDateToString()}\r
Content-Type:application/json; charset=utf-8\r
Path:speech.config\r
\r
...
`);
		...
		while (true) if (messageQueue.length > 0) {
			const message = messageQueue.shift();
			if (message === "close") {
				if (!audioWasReceived) throw new NoAudioReceived("No audio was received.");
				break;
	...
	*/
	async *stream() {
		...
		for (const partialText of this.texts) {
			...
			for await (const message of this._stream()) yield message;
		}
	}
};
function concatUint8Arrays(arrays) {
	...
	const result = new Uint8Array(totalLength);
	let offset = 0;
	for (const arr of arrays) if (arr.length > 0) {
		result.set(arr, offset);
		offset += arr.length;
	}
	return result;
...
var BrowserEdgeTTS = class {
	...
	*/
	async synthesize() {
		...
		for await (const chunk of communicate.stream()) if (chunk.type === "audio" && chunk.data) audioChunks.push(chunk.data);
		else if (chunk.type === "WordBoundary" && chunk.offset !== void 0 && chunk.duration !== void 0 && chunk.text !== void 0) wordBoundaries.push({
			offset: chunk.offset,
...
var BrowserVoicesManager = class _BrowserVoicesManager {
	...
	*/
	static async create(customVoices) {
		...
		manager.voices = (customVoices ?? await listVoices()).map((voice) => ({
			...voice,
	...
	*/
	find(filter) {
		if (!this.calledCreate) throw new Error("BrowserVoicesManager.find() called before BrowserVoicesManager.create()");
...
var SubMaker = class {
	...
	*/
	feed(msg) {
		if (msg.type !== "WordBoundary" || msg.offset === void 0 || msg.duration === void 0 || msg.text === void 0) throw new ValueError("Invalid message type, expected 'WordBoundary' with offset, duration and text");
	...
	*/
	mergeCues(words) {
		if (words <= 0) throw new ValueError("Invalid number of words to merge, expected > 0");
		if (this.cues.length === 0) return;
		const newCues = [];
		let currentCue = this.cues[0];
		for (const cue of this.cues.slice(1)) if (currentCue.content.split(" ").length < words) currentCue = {
			...currentCue,
			end: cue.end,
...
//# sourceMappingURL=edge-tts-universal_browser.js.map